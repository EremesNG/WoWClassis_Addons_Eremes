...
BINDING_NAME_CONROCTOGGLE = "Role Toggle (Damage/Tank/Heals)"
...
--[[		interruptOverlayColor = {
			...
			order = 19,
			set = function(info, r, g, b, a)
				local t = ConROC.db.profile.interruptOverlayColor;
				t.r, t.g, t.b, t.a = r, g, b, a;
			end,
			get = function(info)
				local t = ConROC.db.profile.interruptOverlayColor;
		...
		toggleButtonSize = {
			...
			ConROC_DamageButton:SetScale(ConROC.db.profile.toggleButtonSize)
			ConROC_TankButton:SetScale(ConROC.db.profile.toggleButtonSize)
			ConROC_HealsButton:SetScale(ConROC.db.profile.toggleButtonSize)			
			end,
...
--				ConROCInterruptWindow:SetPoint("RIGHT", "ConROCWindow", "LEFT", -5, 0);
...
--	self.InterruptWindowFrame();
	...
	if Class == 2 or Class == 7 or Class == 11 then
		...
		self:CreateDamageButton();
		self:CreateTankButton();
		self:CreateHealsButton();
	elseif Class == 1 then
		self.DisplayToggleFrame();
		self:CreateDamageButton();
		self:CreateTankButton();
...
function ConROC:PLAYER_TARGET_CHANGED()
	...
	if ConROC.db.profile.enableWindow and (ConROC.db.profile.combatWindow or ConROC:HealSpec()) and ConROC:TarHostile() then
		ConROCWindow:Show();	
	elseif ConROC.db.profile.enableWindow and not (ConROC.db.profile.combatWindow or ConROC:HealSpec()) then
		ConROCWindow:Show();		
	else
		ConROCWindow:Hide();			
	end
...
--ConROC.LEARNED_SPELL_IN_TAB = ConROC.ButtonFetch;
