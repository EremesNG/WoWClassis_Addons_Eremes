...
function TitanPanelGuildButton_BuildForwardPageControl()
	...
	if (numPages > currPage) then
		...
		if (numPages ~= currPage) then
			...
			TitanPanelRightClickMenu_AddCommand(forwardText, TITAN_GUILD_ID, TITAN_GUILD_BUTTON_FORWARDPAGE, L_UIDROPDOWNMENU_MENU_LEVEL);	
		end
	end
end
...
function TitanPanelGuildButton_BuildBackwardPageControl()
	...
	if (currPage ~= 1) then
		...
		TitanPanelRightClickMenu_AddCommand(backwardText, TITAN_GUILD_ID, TITAN_GUILD_BUTTON_BACKWARDPAGE, L_UIDROPDOWNMENU_MENU_LEVEL);	
	end
end
