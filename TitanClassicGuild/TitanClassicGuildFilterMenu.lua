...
function TitanPanelGuildButton_BuildClassFilterMenu()
	...
	local classIndex;
	TitanPanelRightClickMenu_AddTitle(L_UIDROPDOWNMENU_MENU_VALUE, L_UIDROPDOWNMENU_MENU_LEVEL);
	for classIndex = 1, table.getn(classListValues) do
		...
		L_UIDropDownMenu_AddButton(info, L_UIDROPDOWNMENU_MENU_LEVEL+2);
	end
	TitanPanelRightClickMenu_AddCommand(TITAN_GUILD_MENU_HIDE, TITAN_GUILD_ID, TITAN_GUILD_BUTTON_CLOSEMENU, L_UIDROPDOWNMENU_MENU_LEVEL);
end
